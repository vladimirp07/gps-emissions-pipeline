...
def complete_route_v1_optimized(id, registros_person, 
    ...
    # Obtenemos los cachés de aristas planos para ambos grafos
    ...
    while destino_idx < n_registros:
        ...
        ig_actual = ig_walk if es_peaton else ig_drive 
        map_actual = map_walk if es_peaton else map_drive 
        cache_actual = cache_walk if es_peaton else cache_drive
        ...
        if pares_evaluacion:
            # OPTIMIZACIÓN 1: Bulk Query - Agrupar por origen único para consultar en lote a igraph
            ...
            for u, dest_list in org_to_dest.items():
                try:
                    ...
                    # Una sola llamada al motor C de igraph por cada origen único
                    paths_ig = ig_actual.get_shortest_paths(ig_u, to=ig_dests, weights='length', output='vpath')
                    
                    for v, path_ig in zip(valid_dests, paths_ig):
                        if len(path_ig) >= 2:
                            # Traducir IDs a nombres NetworkX
                            route = [ig_actual.vs[nx_id]['_nx_name'] for nx_id in path_ig]
                            bulk_paths[(u, v)] = route
                except Exception:
                    continue
            
            # Evaluamos los pares ordenados por cercanía
            for par in pares_evaluacion:
                u, v = par['origen'], par['destino']
                route = bulk_paths.get((u, v))
                if route is None: continue
                
                # OPTIMIZACIÓN 2 y 3: Medición y agregación única usando caché plano
                distancia_m = 0.0
                time_calc_best = 0.0
                valid_path = True
                tramos_list = []
                ...
                for n in range(len(route)-1):
                    edge_key = (route[n], route[n+1])
                    edge_vals = cache_actual.get(edge_key)
...
def complete_route_v2_optimized(id, registros_person, 
                G_drive, G_walk,
                ig_drive, ig_walk,      
                map_drive, map_walk,    
                geometry_metro=None):
...
class RouteHypothesisEvaluator:
    ...
    def evaluate(self, id_user, df_trip, mode_candidates):
        ...
        for mode in mode_candidates:
            ...
            # complete_route_v1_optimized decide internamente qué red usar según 'modo_transporte'
